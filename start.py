...
def preprocess_data(n_intervals):
    now = df['Time'].max()
    past_6_hours = now - pd.Timedelta(hours=6)

    filtered_data = df[df['Time'] >= past_6_hours]
    df_melted = filtered_data.melt(id_vars='Time', value_vars=['Tm', 'Rh'], 
                                   var_name='Variable', value_name='Value')
    return {
        'filtered_data': filtered_data.to_dict(),
        'melted_data': df_melted.to_dict()
    }
...
def update_time_series_chart(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    
    filtered_data = pd.DataFrame(stored_data['filtered_data'])
    fig = px.line(filtered_data, x='Time', y='Tm', title='Temperature Over Time', markers=True)
    fig.update_layout(
        xaxis_title="Time",
...
def update_temp_humidity_chart(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    df_melted = pd.DataFrame(stored_data['melted_data'])
    fig = px.line(df_melted, x='Time', y='Value', markers=True, color='Variable',
...
def update_scatter_plot(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    ...
    slope, intercept = np.polyfit(filtered_data['Li'], filtered_data['Tm'], 1)
    trendline = np.array(filtered_data['Li']) * slope + intercept
    fig.add_trace(go.Scatter(
        x=filtered_data['Li'], 
        y=trendline, 
        mode='lines', 
...
def update_pm_chart(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    filtered_data = pd.DataFrame(stored_data['filtered_data'])
    fig = go.Figure()
...
def update_VOC_chart(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    filtered_data = pd.DataFrame(stored_data['filtered_data'])
    fig = go.Figure()
...
def update_No2_chart(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    filtered_data = pd.DataFrame(stored_data['filtered_data'])
    fig = go.Figure()
...
def update_Nh3_chart(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    filtered_data = pd.DataFrame(stored_data['filtered_data'])
    fig = go.Figure()
...
def update_Li_chart(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    filtered_data = pd.DataFrame(stored_data['filtered_data'])
    fig = go.Figure()
...
def update_Np_chart(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    filtered_data = pd.DataFrame(stored_data['filtered_data'])
    fig = go.Figure()
...
def update_Na_chart(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    filtered_data = pd.DataFrame(stored_data['filtered_data'])
    fig = go.Figure()
...
def update_H2s_chart(active_tab, stored_data):
    if stored_data is None:
        return dash.no_update
    filtered_data = pd.DataFrame(stored_data['filtered_data'])
    fig = go.Figure()
